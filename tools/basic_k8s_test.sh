...
function getNginxTag() {
	NGX_IMG=
	for node in $(kubectl get node '-o=jsonpath={.items[*].metadata.name}'); do
		TAGS=$(ocne cluster console --direct --node $node -- podman images container-registry.oracle.com/olcne/nginx --format '{{.Tag}}')
		TAG=$(echo "$TAGS" | head -1)
		if [ -n "$NGX_IMG" ]; then
			if [ "nginx:$TAG" != "$NGX_IMG" ]; then
				echo "Warning: $NGX_IMG is not nginx:$TAG"
			fi
		fi
		NGX_IMG="nginx:$TAG"
	done
